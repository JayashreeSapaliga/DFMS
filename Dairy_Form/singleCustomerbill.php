...
<head>
    ...
    <title>GET SINGLE CUSTOMER BILL</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ml-sm-auto col-lg-10 px-md-4 py-2">
                ...
                <div class="d-flex align-items-center p-3 my-2 text-secondary bg-white rounded shadow-sm">
                    <div class="lh-1">
                        <h3 class="h3 mb-0 text-info lh-1"> GET SINGLE CUSTOMER BILL </h1>
                    </div>
                </div>
                <div class="container">
                    <h3 class="text-danger text-center mt-3">SELECT CUSTOMER ID</h3>
                        ...
                        <form method="post">
                            <table class="table">
                                <tr>
                                    ...
                                    <td>
                                        <strong>From Date:</strong> <input type="date" class="mr-3" name="fromdate"
                                            id="fromdate" max=<?php echo date('Y-m-d'); ?> required>
                                    </td>
                                    <td> <strong>To Date:</strong> <input type="date" name="todate" id="todate"
                                            max=<?php echo date('Y-m-d'); ?> required></td>
                                       <td> <input class="btn btn-success ml-3" type="submit" name="submit"
                                            value="Generate"></td>
    ...
    if(isset($_POST["submit"])){
        ...
        <tr> <th>SL No</th> <th> CUSTOMER ID</th> <th>CUSTOMER NAME</th> <th>MILK TYPE</th> <th>TOTAL MILK in LTR</th> <th>TOTAL RUPEES</th> 
            ...
            if ($id && $fromdate && $todate) {
                $presult = mysqli_query($mysql, "SELECT C_id,cu.Name,co.Type,SUM(Qty),SUM(Total) from collection co join customer cu on co.C_id=cu.ID WHERE C_id = '" . $id . "' AND Date BETWEEN '" . $fromdate . "' AND '" . $todate . "' ");
